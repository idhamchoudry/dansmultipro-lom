...
class Edit extends \DansMultiPro\LOM\Controller\Adminhtml\Material
{
    ...
    public function execute()
    {
        ...
        $this->_coreRegistry->register('dansmultipro_lom_material', $model);
        
        // 3. Build edit form
        /** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->resultPageFactory->create();
        $this->initPage($resultPage)->addBreadcrumb(
            $id ? __('Edit Material') : __('New Material'),
            $id ? __('Edit Material') : __('New Material')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Materials'));
        $resultPage->getConfig()->getTitle()->prepend($model->getId() ? __('Edit Material %1', $model->getTitle()) : __('New Material'));
        return $resultPage;
    }
}
